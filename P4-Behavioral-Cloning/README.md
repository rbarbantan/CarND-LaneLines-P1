...
Later I found that version 1 has some issues woth recording the data, so I switched back to the latest released version 
...
All data is collected in the `data` folder, but it is not pushed in this repository.
...
I had to use older versions for some libraries (like `python-socketio` and `python-engineio`)
...
followed by a `Dense(1)` as suggested in the course. 

Since the data consists of only center images form the (mostly circular) first track, 
...
This looks balanced, so the next step is to improve the model a bit. First stop is the simple **LeNet** architecture.
